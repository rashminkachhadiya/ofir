...
@section('title', $catalogueTitle ?? __('Catalogue'))
@section('nav_link')
    <a href="{{ URL::to('/catalogue') }}" class="back-link" aria-label="{{ __('Back to catalogue') }}">
    ...
    <a href="{{ URL::to('/') }}" class="back-link">
        <span>{{ __('Home') }}</span>
    </a>
@endsection
@section('content')
    <x-auth-card :catalogue="true" :title="$catalogueTitle">
        <div class="catalogue-grid">
            @foreach($subCatalogue as $key => $value)
                <div class="minicart-catelogue-button">
                    <a href="{{ URL::to('/catalogue') }}/{{ $catalogueId }}/{{ $key }}">{{ $value }}</a>
                </div>
            @endforeach
        </div>
    </x-auth-card>
